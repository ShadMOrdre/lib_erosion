...
minetest.register_entity("lib_erosion:seedling_x",{
	hp_max = 1,
	visual="wielditem",
	visual_size={x = 1, y = 1},
	...
	on_activate = function(self, staticdata)
		...
		if self.nodename == "lib_erosion:seedling_mound" then
...
minetest.register_entity("lib_erosion:seedling_y",{
	hp_max = 1,
	visual="wielditem",
	visual_size={x = 1, y = 1},
	collisionbox = {0, 0, 0, 0, 0, 0},
...
local remove_item = function(pos, node)
	...
	if node.name == "lib_erosion:seedling_mound" then
	...
	if objs then
		...
		for _, obj in pairs(objs) do
			...
			and obj:get_luaentity().name == "lib_erosion:seedling_x" then
				obj:remove()
			end
			if obj and obj:get_luaentity()
			and obj:get_luaentity().name == "lib_erosion:seedling_y" then
				obj:remove()
			end
		end
	end
...
if not meta then return end
	if meta:get_string("seed") ~= "" then
		...
		local e_x = minetest.add_entity(pos,"lib_erosion:seedling_x")
		local e_y = minetest.add_entity(pos,"lib_erosion:seedling_y")
...
minetest.register_node("lib_erosion:seedling_mound",{
...
if not meta then return end
	...
	on_blast = function(pos, intensity)
		...
		minetest.add_item(pos, {name = "lib_erosion:seedling_mound"})
...
minetest.register_abm({
	nodenames = {"lib_erosion:seedling_mound"},
	interval = 15,
	chance = 1,
	catch_up = false,
...
minetest.register_craft({
	output = 'lib_erosion:seedling_mound',
	recipe = {
		{'', '', ''},
		{'', 'default:dirt', ''},
...
if minetest.get_modpath("mesecons_mvps") then
	...
	--mesecon.register_mvps_stopper("lib_erosion:seedling_mound")
end
